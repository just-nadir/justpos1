...
import { X, Banknote, CreditCard, Smartphone, Check, FileText } from 'lucide-react';
...
const PaymentModal = ({ isOpen, onClose, totalAmount, onPay, selectedCustomer }) => {
  ...
  const handlePayment = () => {
    ...
    if (activeMethod === 'debt' && !selectedCustomer) {
      alert("Nasiya yozish uchun avval mijozni tanlashingiz shart!");
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
      <div className="bg-white w-[500px] rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeMethod === 'debt' && (
             <div className={`text-center text-sm mb-4 p-2 rounded-lg ${selectedCustomer ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600 font-bold'}`}>
               {selectedCustomer 
